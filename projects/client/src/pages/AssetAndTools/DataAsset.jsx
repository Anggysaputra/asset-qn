...
const DataAsset = () => {
  ...
  const [selectedStatusCondition, setSelectedStatusCondition] = useState();
  ...
  const statusConditionGlobal = useSelector((state) => state.status);

  const accessAction = useSelector(
  ...
  let isHiddenAddAsset = true;
  let isHiddenAddTransferAsset = true;
  let HiddenAddReturnAsset = true;
  let isHiddendetailAsset = true;
  let isHiddenEditAsset = true;
  let isHiddenDeleteAsset = true;

  const assetAndToolsEntry = accessAction.find(
    (entry) => entry.name === "Asset & Tools"
  );

  // console.log("assetAndToolsEntry", assetAndToolsEntry);

  if (assetAndToolsEntry && assetAndToolsEntry.m_sub_menus.length > 0) {
    const subMenus = assetAndToolsEntry.m_sub_menus.find(
      (entry) => entry.path === currentPath
    );

    if (subMenus) {
      subMenus.m_actions.forEach((action) => {
        const actionName = action.action; // Menggunakan properti 'action' dari setiap objek aksi
        console.log("actionname", actionName);
        if (action.action === "Add Return Asset") {
          HiddenAddReturnAsset = false;
        } else if (action.action === "Add Transfer Asset") {
          isHiddenAddTransferAsset = false;
        } else if (action.action === "Add Asset") {
          isHiddenAddAsset = false;
        } else if (action.action === "Detail Asset") {
          isHiddendetailAsset = false;
        } else if (action.action === "Edit Asset") {
          isHiddenEditAsset = false;
        } else if (action.action === "Delete Asset") {
          isHiddenDeleteAsset = false;
        }
      });
    }
  }

  console.log("ishiden return", HiddenAddReturnAsset);
  ...
  useEffect(() => {
    if (actionEdit && editedAssetData) {
      ...
      setSelectedStatusCondition(
        editedAssetData ? editedAssetData.m_status_condition : null
      );
  ...
  async function handleSubmit() {
    ...
    const data = {
      ...
      // statusConditionId: selectedStatusCondition?.id || null,
    ...
    if (actionSend === "Edit") {
      data.statusConditionId = selectedStatusCondition?.id || null;
    }
    ...
    <div>
            ...
            <div>
              ...
              {currentStep === 1 && (
                <FormSecondStep
                  ...
                  statusCondition={statusConditionGlobal.statusCondition}
                  selectStatusCondition={selectedStatusCondition}
                  setSelectCondition={setSelectedStatusCondition}
        ...
        <ModalForm2
          ...
          children={
            <TableBodyListReturnAsset
              ...
              isSuperAdmin={isSuperAdmin}
              asset={listTf}
              statusreturn={statusConditionGlobal.statusCondition}
      ...
      <div>
        <AddDataHeaderDataAsset
          ...
          isHiddenAddAsset={isHiddenAddAsset}
          isHiddenAddTransferAsset={isHiddenAddTransferAsset}
          isHiddenAddReturnAsset={HiddenAddReturnAsset}
        ...
        <div>
          <Table2
            className="mb-4"
            hiddenEdit={isHiddenEditAsset}
            hiddenDelete={isHiddenDeleteAsset}
            headCols={[
              // "",
              "Asset Name",
              "Branch",
