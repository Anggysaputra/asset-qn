...
async function getDetailReturn(req, res) {
  try {
    console.log("req params id", req.params);

    const transHId = parseInt(req.params.transHid);

    const transD = await sequelize.query(
      `SELECT 
      ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) no,
      mtd.m_trans_hr_id, mtd.m_category_id ,msc.name ,mtd.m_asset_name, mc.name ctgrName, mtd.no_polisi, mtd.serial_number,
      ms.status_name statusPengiriman, mth.no_return, mth.desc, mth.desc_received, mth.date,
      mcOut.cabang_name cabangOut,
      mcIn.cabang_name cabangIn,
      muSend.username CreatedTransfer,
      muReceived.username userReceived,
      mown.name ownerName,
      count(*) as qty_stock
      FROM m_trans_d_return mtd
      LEFT JOIN m_trans_h_return mth ON mtd.m_trans_hr_id = mth.id
      LEFT JOIN m_owner mown ON mtd.m_owner_id = mown.id
      LEFT JOIN m_users muSend ON mth.createdBy = muSend.id
      LEFT JOIN m_users muReceived ON mth.updatedBy = muReceived.id
      LEFT JOIN m_cabang mcOut ON mth.cabang_id_out = mcOut.id
      LEFT JOIN m_cabang mcIn ON mth.destination = mcIn.id
      LEFT JOIN m_status ms ON mth.m_status_id = ms.id
      LEFT JOIN m_categories mc ON mtd.m_category_id = mc.id
      LEFT JOIN m_status_condition msc ON mtd.m_status_condition = msc.id
      WHERE mtd.m_trans_hr_id = ${transHId}
      GROUP BY CASE WHEN mtd.m_category_id IN (3, 4) THEN mtd.m_asset_name ELSE mtd.id END, 
          mtd.m_status_condition
      HAVING count(*) > 0;
    `,
      {
    ...
    return res
      .status(200)
      .send({ message: "Succefully get detail Return", transD });
  } catch (error) {
    console.log("error", error);
...
async function createReturnAsset(req, res) {
  try {
    ...
    console.log("destinantion", destination);
    ...
    const transH = await db.m_trans_h_return.create({
      destination: branch.dataValues.id,
    ...
    let remainingAssets = [...assetCount]; // Buat salinan assetCount
    ...
    for (const data of processedQtyInputArray) {
      ...
      const matchingAssets = remainingAssets.filter((asset) => {
        return (
...
async function confirmasiReturn(req, res) {
  try {
    ...
    // console.log("branchid login", userBranchIdLogin);
    ...
    console.log("mtransh h", mTransH.dataValues);
    ...
    const statusConditionId = mTransH.dataValues.m_status_return_id;
        ...
        await db.m_assets.update(
          {
            ...
            relocation_status: false,
            updatedBy: user.id,
            m_status_condition_id: statusConditionId,
          },
          { where: { id: assetId } }
        );
      }
