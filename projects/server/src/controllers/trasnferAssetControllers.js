...
async function getDetailTransfer(req, res) {
  try {
    ...
    const transD = await sequelize.query(
      ...
      mtd.m_trans_h_id, mtd.m_category_id ,msc.name ,mtd.m_asset_name, mc.name ctgrName, mtd.no_polisi, mtd.serial_number,
      ms.status_name statusPengiriman, mth.no_transfer, mth.desc, mth.desc_received, mth.date,
    ...
    return res
      .status(200)
      .send({ message: "Succefully get detail transfer", transD });
  } catch (error) {
...
async function createTransferAsset(req, res) {
  try {
    ...
    // console.log("assetCount ya adalah", assetCount);
    ...
    for (const data of processedQtyInputArray) {
      const { selectQty, assetName, coditionAssetId, categoryId, branchAsset } =
        data;
      ...
      const matchingAssets = remainingAssets.filter((asset) => {
        // console.log("asset matching", asset);
        return (
          asset.name === assetName &&
          asset.m_status_condition_id === coditionAssetId &&
          asset.m_category_id === categoryId &&
          asset.relocation_status === false
        );
      });

      // console.log("matching asset adalah", matchingAssets);
      ...
      // Hapus aset yang telah dipilih dari remainingAssets
      ...
      for (const m_asset_id of selectedAssetIds) {
        console.log("mAssetId adalah", m_asset_id);
        console.log("test");
...
async function confirmasiTransfer(req, res) {
  try {
    ...
    console.log("dataTransD loop", dataTransD);
